...
MD = ASSETS / 'draft_cafe_summer_menu_notice_20260516_revised.md'
HTML = ASSETS / 'cafe_summer_menu_tistory_body.html'
TITLE = MD.read_text(encoding='utf-8').splitlines()[0].replace('# ', '').strip()
TAGS = '카페운영,시즌메뉴,품절안내,알레르기안내,카톡문구,소상공인,여름메뉴'
PNG_FILES = [
    ASSETS / '01-thumbnail-cafe-summer-menu.png',
    ASSETS / '02-menu-notice-cafe-summer.png',
    ASSETS / '03-checklist-cafe-summer.png',
]
...
def main():
    ...
    if ('post-editor' not in r.text and 'needCaptcha' not in r.text) or 'login' in r.url.lower():
        raise RuntimeError('newpost HTML sanity failed; not AUTH_BLOCKED without Profile 2 review')
    posts=s.get('https://beemo.tistory.com/manage/posts', timeout=20); posts.raise_for_status()
    if TITLE in posts.text:
        m=re.search(r'href="(/[0-9]+)"[^>]*>[^<]*</a>|https://beemo\.tistory\.com/(\d+)', posts.text)
        print(json.dumps({'status':'DUPLICATE_POSSIBLE','title':TITLE,'url':''}, ensure_ascii=False)); return
    uploads=[]
    for p in PNG_FILES:
        with p.open('rb') as f:
    ...
    if not url:
        ...
        m=re.search(r'href="(/[0-9]+)"', latest) or re.search(r'https://beemo\.tistory\.com/(\d+)', latest)
        url=(('https://beemo.tistory.com'+m.group(1)) if m and m.group(1).startswith('/') else ('https://beemo.tistory.com/'+m.group(1) if m else ''))
