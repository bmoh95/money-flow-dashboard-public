...
MD = ASSETS / 'draft_childcare_vacation_survey_20260516_revised.md'
HTML = ASSETS / 'childcare_vacation_survey_tistory_body.html'
TITLE = MD.read_text(encoding='utf-8').splitlines()[0].replace('# ', '').strip()
TAGS = '어린이집운영,유치원운영,여름방학안내문,등원수요조사,키즈노트,원운영,소상공인'
PNG_FILES = [
    ASSETS / '01-thumbnail-childcare-vacation.png',
    ASSETS / '02-kidsnote-childcare-vacation.png',
    ASSETS / '03-checklist-childcare-vacation.png',
]
...
def find_existing_url(session):
    ...
    # choose first nearby public numeric href after title when possible
    idx=posts.text.find(TITLE)
    window=posts.text[max(0,idx-2000):idx+3000]
